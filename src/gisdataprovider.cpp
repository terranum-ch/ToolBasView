...
bool GISOgrProvider::GISOpen(const wxString &filename) {
  ...
  if (m_Datasource != NULL) {
    ...
    if (m_LayerSpatType == GISSPATIAL_ERROR) return FALSE;
  ...
  wxLogDebug(_T("Open vector failed for file %s, error msg is : %s"), filename.c_str(), myErrMsg.c_str());
  return FALSE;
}
...
long GISDBProvider::GISGetFeatureCount() {
  wxString sSentence = wxString::Format(_T("SELECT  COUNT(*) FROM %s"), m_LayerName.c_str());
...
bool GISDBProvider::GISComputeIndex(const wxArrayString &fields, const wxString &table) {
  wxString sSentence = wxString::Format(_T("CREATE SPATIAL INDEX sp_index ON %s (OBJECT_GEOMETRY);"), table.c_str());
  ...
  if (bComputeExtend) {
    ...
    if (GISComputeBoundingBox(wkbstring, &myEnveloppe)) {
      sSentence = wxString::Format(
          _T("INSERT INTO GENERIC_LINES ")
          _T("(WKT_GEOMETRY, MINX, MINY, MAXX, MAXY)")
          _T("VALUES (\"%s\", %f, %f, %f, %f)"),
          wkbstring.c_str(), myEnveloppe.MinX, myEnveloppe.MinY, myEnveloppe.MaxX, myEnveloppe.MaxY);
    }

  } else {
    sSentence = wxString::Format(_T("INSERT INTO GENERIC_LINES (WKT_GEOMETRY) VALUES (\"%s\")"), wkbstring.c_str());
  }
...
bool GISDBProvider::GISSetFeatureAsWkTBuffer(const wxArrayString &featurelist, bool bComputeExtend) {
  ...
  for (i = 0; i < featurelist.GetCount(); i++) {
    sSentence.Append(
        wxString::Format(_T("INSERT INTO %s")
                         _T(" VALUES (-1, GeomFromText('%s')); "),
                         m_LayerName.c_str(), featurelist.Item(i).c_str()));
  }
...
OGRGeometry *GISDBProvider::GISGetFeatureByBuffer(const double &x, const double &y, const int &ibuffer,
                                                  int &iFidFound) {
