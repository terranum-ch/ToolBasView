...
DELETETABLEDATA_DLG::DELETETABLEDATA_DLG(wxWindow* parent, wxWindowID id, DataBase* database, const wxString& caption,
  ...
  if (m_pDatabase->DataBaseGetResults(myListTables) == false) return;

  m_DlgDel_ListTables->Append(myListTables);
}

bool DELETETABLEDATA_DLG::Create(wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos,
...
void DELETETABLEDATA_DLG::OnPressOK(wxCommandEvent& event) {
  ...
  for (unsigned int i = 0; i < m_DlgDel_ListTables->GetCount(); i++) {
    if (m_DlgDel_ListTables->IsChecked(i)) m_SelectedTables.Add(m_DlgDel_ListTables->GetString(i));
  }
  ...
  for (unsigned j = 0; j < m_SelectedTables.GetCount(); j++) {
    ...
    if (m_DlgDel_Chk_Data->IsChecked()) {
      sSentence.Append(wxString::Format(_T("DELETE FROM %s; "), (m_SelectedTables.Item(j)).c_str()));
    }

    // do we have to drop the tables
    if (m_DlgDel_Chk_table->IsChecked()) {
      sSentence.Append(wxString::Format(_T("DROP TABLE %s; "), (m_SelectedTables.Item(j)).c_str()));
    }
  }
...
void DELETETABLEDATA_DLG::CreateControls() {
  ...
  m_DlgDel_ListTables = new wxCheckListBox(itemDialog1, ID_DLGDEL_TABLES, wxDefaultPosition, wxSize(250, 200),
  ...
  m_DlgDel_Chk_Data =
      new wxCheckBox(itemDialog1, ID_DLGDEL_CHK_DATA, _("Deleting data"), wxDefaultPosition, wxDefaultSize, 0);
  m_DlgDel_Chk_Data->SetValue(true);
  itemStaticBoxSizer6->Add(m_DlgDel_Chk_Data, 0, wxALIGN_LEFT | wxALL, 5);

  m_DlgDel_Chk_table =
      new wxCheckBox(itemDialog1, ID_DLGDEL_CHK_TABLE, _("Deleting table"), wxDefaultPosition, wxDefaultSize, 0);
  m_DlgDel_Chk_table->SetValue(false);
  itemStaticBoxSizer6->Add(m_DlgDel_Chk_table, 0, wxALIGN_LEFT | wxALL, 5);

  m_DlgDel_Chk_Vacuum = new wxCheckBox(itemDialog1, ID_DLGDEL_VACUUM, _("Cleaning dataspace (VACUUM)"),
  ...
  wxStaticLine* itemStaticLine11 =
      new wxStaticLine(itemDialog1, ID_STATICLINE2, wxDefaultPosition, wxDefaultSize, wxLI_HORIZONTAL);
