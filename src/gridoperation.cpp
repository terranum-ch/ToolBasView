...
void GridOperation::GridOpSetNumberOfColumn(int iNumber) {
  ...
  // delete all rows...
  if (pGrid->GetNumberRows() > 0) pGrid->DeleteRows(0, pGrid->GetNumberRows(), FALSE);
}
...
void GridOperation::GridClear(int iColumn, int iRows) {
  int myColumnWidth = pGrid->GetColSize(0);
  pGrid->BeginBatch();
  ...
  for (int i = 0; i < pGrid->GetNumberCols(); i++) {
    pGrid->SetColSize(i, myColumnWidth);
  }
