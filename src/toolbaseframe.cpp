...
void TBVFrame::_CreateControls() {
  ...
  m_mgr.AddPane(m_mainpanel, wxAuiPaneInfo()
                                 .Name(wxT("main"))
                                 .Left()
  ...
  m_mgr.AddPane(m_tocpanel, wxAuiPaneInfo()
                                .Name(wxT("toc"))
                                .Left()
                                .Caption(wxT("Table of content"))
                                .CloseButton(false)
                                .Dock()
                                .Resizable()
  ...
  m_mgr.AddPane(m_querypanel, wxAuiPaneInfo()
                                  .Name(wxT("query"))
                                  .Bottom()
                                  .Caption(wxT("Query"))
                                  .CloseButton(false)
  ...
  m_QueryTxtCtrl->SetKeyWords(
      0,
      _T("accessible add all alter analyze and as asc asensitive before between bigint binary blob both by call ")
      _T("cascade case change char character check collate column condition constraint continue convert create cross ")
      _T("current_date current_time current_timestamp current_user cursor database databases day_hour day_microsecond ")
      _T("day_minute day_second dec decimal declare default delayed delete desc describe deterministic distinct ")
      _T("distinctrow div double drop dual each else elseif enclosed escaped exists exit explain false fetch float ")
      _T("float4 float8 for force foreign from fulltext general grant group having high_priority hour_microsecond ")
      _T("hour_minute hour_second if ignore ignore_server_ids in index infile inner inout insensitive insert int int1 ")
      _T("int2 int3 int4 int8 integer interval into is iterate join key keys kill leading leave left like limit ")
      _T("linear lines load localtime localtimestamp lock long longblob longtext loop low_priority ")
      _T("master_heartbeat_period master_ssl_verify_server_cert match maxvalue mediumblob mediumint mediumtext ")
      _T("middleint minute_microsecond minute_second mod modifies natural not no_write_to_binlog null numeric on ")
      _T("optimize option optionally or order out outer outfile precision primary procedure purge range read reads ")
      _T("read_write real references regexp release rename repeat replace require resignal restrict return revoke ")
      _T("right rlike schema schemas second_microsecond select sensitive separator set show signal slow[d] smallint ")
      _T("spatial specific sql sqlexception sqlstate sqlwarning sql_big_result sql_calc_found_rows sql_small_result ")
      _T("ssl starting straight_join table terminated then tinyblob tinyint tinytext to trailing trigger true undo ")
      _T("union unique unlock unsigned update usage use using utc_date utc_time utc_timestamp values varbinary ")
      _T("varchar varcharacter varying when where while with write xor year_month zerofill   general ")
      _T("ignore_server_ids master_heartbeat_period maxvalue resignal signal slow"));
  ...
  m_QueryShowCtrl =
      new wxButton(m_querypanel, ID_BTN_SHOW_RESULTS, _("Show Results..."), wxDefaultPosition, wxDefaultSize, 0);
  bSizer7->Add(m_QueryShowCtrl, 0, wxALL, 5);

  bSizer7->Add(0, 0, 1, wxEXPAND, 5);

  m_AddToListBtnCtrl =
      new wxButton(m_querypanel, ID_BTN_ADD_TO_LIST, _("Add to List..."), wxDefaultPosition, wxDefaultSize, 0);
  ...
  m_mgr.AddPane(m_logpanel, wxAuiPaneInfo()
                                .Name(wxT("log"))
                                .Bottom()
                                .Caption(wxT("Log"))
                                .CloseButton(false)
  ...
  m_mgr.AddPane(m_QueryListPanel, wxAuiPaneInfo()
                                      .Name(wxT("querylist"))
                                      .Right()
                                      .Caption(wxT("Query List"))
                                      .CloseButton(false)
                                      .Dock()
                                      .Resizable()
                                      .FloatingSize(wxDefaultSize)
  ...
  m_QueryListTreeCtrl = new QueryListTree(m_QueryListPanel, ID_QUERY_LIST_TREE, wxDefaultPosition, wxDefaultSize,
...
void TBVFrame::_CreateMenu() {
  ...
  ); item2->Append(ID_MENU_SHOW_LOG, _("Show Log panel\tCtrl+L"), _("Show the log panel")); item2->AppendSeparator();
  item2->Append( ID_MENU_SPATIAL_ADD, _("Add spatial data into the database...\tCtrl-S"), _("Allow user to load spatial
  data into a database table (SHP)") ); item2->Append( ID_MENU_SPATIAL_SEARCH, _("Search spatial data...\tCtrl-F"),
  _("Search spatial data used for benchmark") ); item2->Append(ID_MENU_PROCESS_SQL_FILE, _("Process SQL
  file...\tCtrl+Alt+P"), wxEmptyString); item2->AppendSeparator(); item2->Append( ID_MENU_DELETE, _("Delete data from
  database...\tCtrl-Del"),_("Dialog for deleting data from the database") ); item2->AppendSeparator();
...
void TBVFrame::OnSpatialDataAdd(wxCommandEvent& event) {
  ...
  wxProgressDialog myPrgDlg(_("Importing data into the Database"), _("Importing in progress, please wait..."), iMax);
  ...
  // show elapsed time
  wxLogMessage(_("Elapsed time for adding %d spatial data is : %u [ms]"), lFeatureCount, sw.Time());
...
void TBVFrame::OnDoubleClickListe(wxTreeEvent& event) {
  ...
  if (myFieldArray.Count() > 0) {
    // debug message...
    wxLogMessage(_("%d fields readed from the table '%s'"), (int)myFieldArray.Count(), myTempString.c_str());
    ...
    if (m_Database.DataBaseQuery(_T("SELECT * FROM ") + myTempString)) {
      ...
      while (m_Database.DataBaseGetNextResult(myFieldArray)) {
        ...
        // add a new line
        m_GridOp->GridOpAddDataRow(iArrayCount, &myFieldArray);
      }
      m_Database.DataBaseClearResults();
...
void TBVFrame::OnNewDataBase(wxCommandEvent& event) {
  ...
  NEWDBASE_OP2* myDlg = new NEWDBASE_OP2(this, -1, _("Create a new database"), wxDefaultPosition, wxDefaultSize);
...
void TBVFrame::OnDisplayStatistics(wxCommandEvent& event) {
  ...
  wxMessageBox(myText, _("Database statistics"), wxOK | wxICON_INFORMATION, this);
}
...
void TBVFrame::OnProcessSQLFile(wxCommandEvent& event) {
  wxFileDialog myOpenDlg(this, _("Open SQL file"), "", "", _("SQL file (*.sql)|*.sql"),
...
void TBVFrame::OnImportTXTFile(wxCommandEvent& event) {
  wxFileDialog myOpenDlg(this, _("Open TXT file"), "", "", _("txt file (*.txt)|*.txt"),
                         wxFD_OPEN | wxFD_FILE_MUST_EXIST);
  if (myOpenDlg.ShowModal() != wxID_OK) {
    return;
  }
  ...
  wxString myQuery = wxString::Format(_T("CREATE TABLE `%s` (%s) ENGINE=MyISAM  DEFAULT CHARSET=utf8;"),
                                      myTxtFileName.GetName(), myQueryCols);
...
void TBVFrame::OnBtnHistory(wxCommandEvent& event) {
  wxString myHistoryValue = wxGetSingleChoice(_("SQL command history"), _("History"), *(GetHistory()), this,
                                              wxDefaultCoord, wxDefaultCoord, false, 500, 500);
